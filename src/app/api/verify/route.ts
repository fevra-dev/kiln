...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof accountData === 'object' && accountData !== null && 'parsed' in accountData) {
      const parsedData = accountData as {
        parsed?: { info?: { supply?: string; decimals?: number }; type?: string };
      };
      ...
      (accountData && typeof accountData !== 'string' && 'byteLength' in accountData)
    ) {
          ...
          return NextResponse.json(
            {
              success: false,
              error:
                'Account data too short to be a valid mint. Please provide a valid NFT mint address.',
            },
            { status: 400 },
          );
        }
      ...
      for (const sigInfo of signatures) {
        try {
          ...
          const compiledInstructions =
            msgAny['compiledInstructions'] ?? msgAny['instructions'] ?? [];
            ...
            try {
              ...
              try {
                ...
                try {
                  ...
                  if (
                    (memoJson.standard === 'Kiln' || memoJson.standard === 'KILN') &&
                    memoJson.solana?.mint === validated.mint
                  ) {
                    if (
                      (memoJson.action === 'teleburn-seal' || memoJson.action === 'seal') &&
                      ...
                      (memoJson.action === 'teleburn' ||
                        memoJson.action === 'teleburn-burn' ||
                        memoJson.action === 'teleburn-incinerate' ||
                        memoJson.action === 'teleburn-derived' ||
                        memoJson.action === 'burn' ||
                        memoJson.action === 'incinerate' ||
                        memoJson.action === 'retire') &&
                      memoJson.inscription?.id
                    ) {
                      ...
                      if (memoJson.timestamp) {
                        teleburnTimestamp =
                          typeof memoJson.timestamp === 'number'
                            ? memoJson.timestamp
                            : parseInt(memoJson.timestamp, 10);
                      } else {
                        teleburnTimestamp = tx.blockTime ?? undefined;
                      }
                      burnSignature = sigInfo.signature;
