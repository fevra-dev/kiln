...
export async function isPNFT(
  ...
  try {
    ...
    console.log(
      `🔍 PNFT CHECK: Starting pNFT detection (server-side: ${typeof window === 'undefined'})`,
    );

    console.log(
      `🔍 PNFT CHECK: Starting comprehensive pNFT detection for mint: ${mint.toBase58()}`,
    );
    ...
    if (owner) {
      try {
        const { TOKEN_PROGRAM_ID, getAssociatedTokenAddressSync } = await import(
          '@solana/spl-token'
        );
        ...
        if (tokenAccountInfo) {
          ...
          if (tokenAccountData.isFrozen && tokenAccountData.amount > 0n) {
            console.log(
              `🔍 PNFT CHECK: Detected frozen token account with tokens - likely a pNFT!`,
            );
            return true;
          }
        } else {
